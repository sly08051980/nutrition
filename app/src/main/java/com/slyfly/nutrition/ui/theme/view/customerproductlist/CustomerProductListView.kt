...
fun CustomerProductListView(
    ...
    vmScanner : ResultScannerViewModel
) {
    ...
    val itemScanner = DataItemBody(context).scanner
    LazyVerticalGrid(
...
item() {
    Card(
        onClick = {
            vmScanner.scanAndFetchProduct(context){
                navController.navigate(View.ScannerResult.title)
            }} ,
        elevation = CardDefaults.cardElevation(4.dp),
        modifier = Modifier.padding(4.dp)
    ){
        Image(painter = painterResource(id= R.drawable.codebarre), contentDescription = "scanner",modifier=Modifier.padding(8.dp))
        Text(text = "Scanner",modifier=Modifier.padding(8.dp),)
    }
}
